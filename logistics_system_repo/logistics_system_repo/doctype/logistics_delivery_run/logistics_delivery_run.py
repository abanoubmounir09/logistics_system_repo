...
class LogisticsDeliveryRun(Document):
	# ------------------------------------------------------------------ #
	...
	def build_run(self, driver: str, order_names: list | None = None):
		...
		driver_doc = frappe.get_doc("Logistic Driver", driver)
		...
		for order in orders:
			order_doc = frappe.get_doc("Logistics Order", order.name)
	...
	#logistics_system_repo.logistics_system_repo.doctype.logistics_delivery_run.logistics_delivery_run.py
	@frappe.whitelist()
	def manual_build_run(self, driver: str, order_names: list | None = None):
		"""
		Assign a set of Open orders to a driver as an ordered list of stops.
		If order_names is not provided, the oldest Open orders (by priority,
		then creation) up to the driver's max_stops_per_run are auto-selected.
		"""
		if self.run_status != "Draft" :
			frappe.throw(_("This run has already been built."))
	...
	def _select_orders_for_run(self, driver_doc, order_names):
		...
		if order_names:
			...
			orders = [frappe.get_doc("Logistics Order", name) for name in order_names]
			for order in orders:
	...
	def start_run(self):
		...
		for stop in self.delivery_stops:
			order_doc = frappe.get_doc("Logistics Order", stop.related_order)
			order_doc.status = "En Route"
			order_doc.save()

		driver_doc = frappe.get_doc("Logistic Driver", self.driver)
	...
	def _get_stop(self, stop_name):
		for stop in self.delivery_stops:
			if stop.related_order == stop_name:
				return stop
